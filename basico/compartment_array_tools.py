...
def plot_linear_time_course(data, dm, prefix=None, shading='gouraud', min=np.nan, max=np.nan):
    """
        ...
        for metab in metab_names:
            ...
            for x in range(num_x):
                for y in range(num_y):
                    arr[x, y] = cur[metab + '{' + prefix + '[{0},{1}]'.format(x_range[x], y_range[y]) + '}']
            fig, ax = plt.subplots()
...
def plot_rectangular_time_course_2(data, dm, times=None, metab_names=None, prefix=None, shading='gouraud', min=np.nan, max=np.nan):
    """
        Plots the simulation data on the loaded model assuming that this is in the format
        of an array of compartments as generated by COPASI. This will create a figure for each species
        at each of the specified time points.
    ...
    :param prefix: string of compartment prefix to indicate which compartment should be visualized. This is
                   expected to not include the indices, so it would be 'compartment' rather than 'compartment[0]'
    :param metab_names: the metabolites to draw, defaults to all
    :param shading: optional shading for the color mesh, defaults to 'gouraud' can also be 'flat'
    :param min: optional min range, defaults to NaN, meaning that it is to be the minimum value of the data
    :param max: optional max range, defaults to NaN, meaning that it is to be the maximum value of the data
    ...
    for i in range(len(metab_names)):
        color_scheme[metab_names[i]] = schemes[i % len(schemes)]
    ...
    for i in range(len(data)):
        ...
        for metab in metab_names:
            ...
            for x in range(num_x):
                for y in range(num_y):
                    arr[x, y] = cur[metab + '{' + prefix + '[{0},{1}]'.format(x_range[x], y_range[y]) + '}']
            mesh = ax.pcolormesh(x_range, y_range, arr, shading=shading, cmap=plt.get_cmap(color_scheme[metab]))
            vmin = np.min(arr) if np.isnan(min) else min
            vmax = np.max(arr) if np.isnan(max) else max
            mesh.set_clim(vmin=vmin, vmax=vmax)
            fig.colorbar(mesh, ax=ax)
...
def animate_rectangular_time_course(data, dm, metab=None, prefix=None, shading='gouraud', min=np.nan, max=np.nan, filename=None):
    """
        Plots the simulation data on the loaded model assuming that this is in the format
        of an array of compartments as generated by COPASI. This will create a figure for each species
        at each of the specified time points.
    ...
    def _get_ith_set(series, species):
        a_data = np.zeros((num_x, num_y)) * np.nan
        for x in range(num_x):
            for y in range(num_y):
                cur_species = species + '{' + prefix + '[{0},{1}]'.format(x_range[x], y_range[y]) + '}'
                if cur_species in series:
                    a_data[x, y] = series[cur_species]

        return a_data

    fig, ax = plt.subplots()
    arr = _get_ith_set(data.iloc[0], metab)
    mesh = ax.pcolormesh(x_range, y_range, arr, shading=shading)
    vmin = np.min(arr) if np.isnan(min) else min
    vmax = np.max(arr) if np.isnan(max) else max
    mesh.set_clim(vmin=vmin, vmax=vmax)
    ...
    def _plot_ith_set(i):
        arr = _get_ith_set(data.iloc[i], metab)
        vmax = np.max(arr) if np.isnan(max) else max
        vmin = np.min(arr) if np.isnan(min) else min
...
if __name__ == "__main__":
    ...
    #
    dm = load_example('brusselator')
    create_rectangular_array(dm, 30, 30, ['X', 'Y'], [0.16, 0.8], delete_template=True)
